...
data = np.transpose(np.loadtxt("timesg.tpt"))
x = data[0]
y = data[1] #data[1,::2]
...
coefficients1 = np.abs(np.append(opt.curve_fit(f1, x[67:], y[67:],maxfev=1000000)[0], []))
...
plt.plot(new_x, new_y22)
#plt.plot(new_x, new_y43)
#plt.plot(x, e_fct)

# Write function values in file
out = np.transpose([x,y/1000,e_fct/1000,new_y1/1000,new_y2/1000,new_y3/1000,new_y4/1000,new_y22/1000])
f = open("functionvaluesg.csv", "w")
f.write("n t exp p1 p2 p3 p4 p22 \n")
for x in out:
    for y in x:
        f.write(str(y))
        f.write(" ")
